...
public class ModelLoader
{
    ...
    public string Predict(string text)
    {
        ...
        {
            // Extract the last user message from the prompt if possible
            var lastUserIndex = text.LastIndexOf("User: ");
            if (lastUserIndex != -1)
            {
                var lastAiIndex = text.LastIndexOf("AI:");
                var userMessage = "";
                if (lastAiIndex > lastUserIndex)
                {
                    userMessage = text.Substring(lastUserIndex + 6, lastAiIndex - lastUserIndex - 6).Trim();
                }
                else
                {
                    userMessage = text.Substring(lastUserIndex + 6).Trim();
                }
