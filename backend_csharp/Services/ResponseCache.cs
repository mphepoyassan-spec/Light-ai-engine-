...
public class ResponseCache
{
    ...
    private readonly ConcurrentQueue<(string Key, DateTime Timestamp)> _keys = new();
    ...
    public void Set(string key, string value)
    {
        ...
        _keys.Enqueue((key, timestamp));

        while (_cache.Count > _maxSize)
        {
            if (_keys.TryDequeue(out var oldest))
            {
                // Only remove if it's the same version of the key we enqueued
                // (prevents evicting a key that was just updated)
