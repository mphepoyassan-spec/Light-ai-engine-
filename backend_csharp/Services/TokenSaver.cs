...
public class TokenSaver
{
    ...
    private static readonly Regex _fillerRegex = new($@"\s*\b({string.Join("|", _fillerWords)})\b\s*", RegexOptions.IgnoreCase | RegexOptions.Compiled);
    private static readonly Regex _whitespaceRegex = new(@"\s+", RegexOptions.Compiled);
    private static readonly Regex _punctuationRegex = new(@"([!?.,]){2,}", RegexOptions.Compiled);
    private static readonly Regex _leadingNonWordRegex = new(@"^\W+", RegexOptions.Compiled);
