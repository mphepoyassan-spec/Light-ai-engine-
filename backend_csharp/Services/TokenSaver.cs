...
public class TokenSaver
{
    ...
    private static readonly string[] FillerWords = { "uh", "um", "er", "ah", "like", "you know", "basically", "actually" };
    private static readonly Regex FillerRegex = new Regex($@"\s*\b({string.Join("|", FillerWords)})\b\s*", RegexOptions.IgnoreCase | RegexOptions.Compiled);
    private static readonly Regex WhitespaceRegex = new Regex(@"\s+", RegexOptions.Compiled);
    private static readonly Regex PunctuationRegex = new Regex(@"([!?.,]){2,}", RegexOptions.Compiled);
    private static readonly Regex LeadingNonWordRegex = new Regex(@"^\W+", RegexOptions.Compiled);
